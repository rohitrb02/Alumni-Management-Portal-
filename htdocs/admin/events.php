					...
					<div class="card-body">
						<table class="table table-condensed table-bordered table-hover">
							...
							<tbody>
								<?php
								$i = 1;
								$alumni = $conn->query("SELECT * from events  WHERE status ='0'");
...
if (isset($_POST['approve'])) {
	$id = $_POST['id'];
	$select = "UPDATE events SET status='1' where id='$id'";
...
if (isset($_POST['deny'])) {
	$id = $_POST['id'];
	$select = "DELETE from events where id='$id'";
	$result = mysqli_query($conn, $select);
